...
def run_trials_true_gmm(true_system, init_state_model, test_data, save_directory, num_trials,
                        ...
                        max_time=2000,
                        save_figures=True,
                        traj_data_for_plotting=None):
    """
    Run multiple trials of belief propagation using the true system model and evaluate on test data.
    
    Parameters:
    -----------
    true_system : System
        The true system model to use as the transition model.
    init_state_model : GMModel
        The initial state GMM model.
    ...
    max_mixands : int
        Maximum number of mixands before stopping (for wsasos, default: 4000).
    max_time : float
    ...
    traj_data_for_plotting : list of np.ndarray, optional
        Trajectory data to use for plotting (if None, uses test_data, default: None).
    ...
    dim = test_data[0].shape[1]
    num_timesteps = len(test_data)  # Number of timesteps in test data (includes t=0)
    
    # Use test_data for plotting if traj_data_for_plotting not provided
    if traj_data_for_plotting is None:
        traj_data_for_plotting = test_data
    ...
    for trial in range(num_trials):
        ...
        # Use the true system as transition model
        transition_model = true_system
            ...
            try:
                if propagation_method == 'ekf':
                    ...
                    if (p_curr.n_mixands() > max_mixands):
                        print(f"Stopping early at t={k} due to max_mixands limit")
        ...
        if trial == 0 and save_figures:
            ...
            state_dist_fig, _ = state_distribution_plot_2D(traj_data_for_plotting, pdf_plotter, 
                                                           interactive=False, bounds=x_bounds)
            particle_figs, pdf_figs = state_distribution_plot_2D(traj_data_for_plotting, pdf_plotter, 
                                                                  interactive=False, bounds=x_bounds, 
...
if __name__ == "__main__":
    ...
    init_state_model = GMModel(means=[np.array([0.2, 0.1])], covariances=[np.diag([0.2, 0.2])], weights=[1.0])
    ...
    # Grid method
    print("\n" + "="*60)
    print("Running Grid propagation method (TRUE system)")
    print("="*60)
    log_likelihoods_grid, prop_times_grid = run_trials_true_gmm(
        system, init_state_model, test_traj_data,
        f"./benchmarks/trajectory_2D_TRUE_grid_{curr_date_time}",
        num_trials=1,
        ...
        max_time=max_time,
        save_figures=True,
        traj_data_for_plotting=traj_data
    )
    
    # EKF method
    print("\n" + "="*60)
    print("Running EKF propagation method (TRUE system)")
    print("="*60)
    log_likelihoods_ekf, prop_times_ekf = run_trials_true_gmm(
        system, init_state_model, test_traj_data,
        f"./benchmarks/trajectory_2D_TRUE_ekf_{curr_date_time}",
        num_trials=1,
        propagation_method='ekf',
        x_bounds=x_bounds,
        max_mixands=max_mixands,
    ...
    # WSASOS method
    print("\n" + "="*60)
    print("Running WSASOS propagation method (TRUE system)")
    print("="*60)
    log_likelihoods_wsasos, prop_times_wsasos = run_trials_true_gmm(
        system, init_state_model, test_traj_data,
        f"./benchmarks/trajectory_2D_TRUE_wsasos_{curr_date_time}",
        num_trials=1,
