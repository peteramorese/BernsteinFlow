...
from .sos_experiment_TEST4D import run_trials_sos
...
if __name__ == "__main__":
    ...
    print("Benchmarking 4D CartPole system")

    # System model (taken from sos_experiment_TEST4D)
    system = CartPole(
        dt=0.05,
        m_c=2.0,
        m_p=0.1,
    ...
    # Number of trajectories (SOS parameters from sos_experiment_TEST4D)
    n_traj_train_sos = 1000
    # Use a smaller dataset for GPGMM / NF by default (can be tuned as needed)
    n_traj_train_gpgmm = 400
    n_traj_train_nf = 1000  # NF uses same amount as SOS
    n_traj_test = 10000

    # Number of training epochs (from sos_experiment_TEST4D)
    n_epochs_init = 200
    n_epochs_tran = 100
    ...
    # Grid method parameters for 4D CartPole: [x, x_dot, theta, theta_dot]
    x_bounds_4d = [
        -10.0,
        10.0,   # cart position
        -10.0,
    ...
    # SOS hyperparameters (from sos_experiment_TEST4D)
    sos_tran_params = {
        "min_alpha_beta": 0.00,
    ...
    # Generate data
    traj_data_train_sos = sample_trajectories(system, init_state_sampler, training_timesteps, n_traj_train_sos)
    ...
    # Create initial state model for true_gmm (single Gaussian matching init_state_sampler)
    init_mean = np.array([0.0, 5.0, 0.0, 0.0])
    init_cov = np.diag([0.1, 0.1, 0.1, 0.1])
    ...
    benchmark_dir = os.path.join("benchmarks", f"benchmark_4D_{curr_date_time}")
    ...
    if "sos" in methods_to_run:
        ...
        print("=" * 60)
        # Run SOS experiments (CartPole-specific, via sos_experiment_TEST4D.run_trials_sos)
        sos_ll, sos_prop_times = run_trials_sos(
            ...
            n_epochs_tran_fine=n_epochs_tran_refine,
            tran_params=sos_tran_params,
            init_params=sos_init_params,
            save_figures=True,
        )
