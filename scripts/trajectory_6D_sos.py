...
def plot_2d_marginals_over_time(beliefs_list, keep_pair, pair_name,
    ...
    with torch.no_grad():
        for belief in beliefs_list:
            ...
            grids.append(Z)
            max_density = max(max_density, float(np.max(Z)))
    ...
    for k, Z in enumerate(grids):
        ...
        c = k % n_cols
        ax = axes[r][c]
        cf = ax.contourf(XX, YY, Z, levels=30, vmin=0.0, vmax=max_density)
        ax.set_title(f"t={k}")
...
if __name__ == "__main__":
    ...
    gdt = GaussianDistTransform.moment_match_data(np.vstack(traj_data), variance_pads=[4.2, 4.2, 2.1, 4.2, 4.2, 2.1])
    ...
    transition_model = SumBetaSOSModel(dy=dim, dx=dim, n=n, n_terms=n_terms, min_alpha_beta=0.1, max_alpha_beta=40.0, mu=0.1, min_Q_eigval=1e-8, regularization_weight=1e-3)

    print("Training transition model...")
    transition_model.to(device=device, dtype=DTYPE)
    trans_optimizer = torch.optim.Adam(transition_model.parameters(), lr=1e-2)
    optimize(transition_model, Up_dataloader, trans_optimizer, epochs=200)
    ...
    optimize(init_state_model, U0_dataloader, trans_optimizer, epochs=200)
    ...
    #plot_2d_particle_scatter_over_time(u_traj_data, (0, 1), "px_pz_particles",
    #                                   sample_limit=10000,
    #                                   save_path="figures/mc_particles_px_pz.png",
    #                                   show_plot=True)
    #plot_2d_particle_scatter_over_time(u_traj_data, (3, 4), "vx_vz_particles",
    #                                   sample_limit=10000,
    #                                   save_path="figures/mc_particles_vx_vz.png",
    #                                   show_plot=True)
    #plot_2d_particle_scatter_over_time(u_traj_data, (2, 5), "theta_omega_particles",
    #                                   sample_limit=10000,
    #                                   save_path="figures/mc_particles_theta_omega.png",
    #                                   show_plot=True)
